...
class Activity {
  ...
  static Future<Response?> create(
      BuildContext context, List<Map<String, dynamic>> activities) async {
    final webService = WebService(context);
    webService
        .setMethod('POST')
        .setEndpoint('identity/user-biodata-activities/update');
  ...
  static Future<Activity?> fetchOne(BuildContext context, int id) async {
    ...
    var response = await webService
        .setEndpoint('identity/user-biodata-activities/$id')
        .send();
    if (response == null) return null;
    if (response.status) {
      final parseList = jsonDecode(response.body!).cast<Map<String, dynamic>>();
      List<Activity> bioDatas =
