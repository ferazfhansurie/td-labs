...
class CategoryRobot {
  const CategoryRobot(this.tester);

  final WidgetTester tester;
  Future<void> initMall() async {
    await tester.pumpAndSettle();
    expect(find.byKey(const Key("category_screen")), findsWidgets);
  }
  ...
  Future<void> tapCategoryItem() async {
    final itemFinder = find.byKey(const Key("category_tap 0"));
